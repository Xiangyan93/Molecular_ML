...
def df_filter(df, ratio=None, seed=0, property=None, min=None, max=None,
              std=None, score=None):
    np.random.seed(seed)
    N = len(df)
    if min is not None:
        df = df.loc[df[property] > min]
    if max is not None:
        df = df.loc[df[property] < max]
    if std is not None:
...
def get_XY_from_df(df, kernel_config, coef=False, property=None):
    ...
    if coef:
        ...
        Y = np.array(df['coeffs'].apply(fun).to_list())
    else:
...
def read_input(csv, property, result_dir, theta=None, seed=0, optimizer=None,
    ...
    if coef:
        ...
        train_X, train_Y = get_XY_from_df(df_train, kernel_config,
                                          property=property)
        test_X, test_Y = get_XY_from_df(df_test, kernel_config,
                                        property=property)
    if test_X is None:
    ...
    return df_train, df_test, train_X, train_Y, test_X, test_Y, kernel_config
...
def main():
    ...
    if args.alpha_outlier:
        ...
        learner = Learner(train_X, train_Y, test_X, test_Y,
                          kernel_config.kernel, seed=args.seed,
                          alpha=args.alpha,
                          optimizer=optimizer)
        learner.train()
        gpr = learner.model
    ...
    if args.mode == 'loocv':  # directly calculate the LOOCV
        ...
        learner = Learner(train_X, train_Y, test_X, test_Y,
                          kernel_config.kernel, seed=args.seed, alpha=alpha,
                          optimizer=optimizer)
        learner.train()
